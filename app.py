...
st.markdown('Investing in stock market is not as complicated as it sounds. But like every thing in this world, simple things are not the most easiest things to do. Millions of people who want to be like Buffet know that his principles are simple but not easy to replicate. One way of replicating is investing in well run companies and sitting on it tightly for atleast 10 years without churning the portfolio which is the concept of Coffee Can Investing.')
...
#st.write('You selected:', companies_list)
...
updated = pd.DataFrame({'Companies' : companies_list, 
			'CAGR(10 Years)' : list(data[data.sort_values(by=['Companies'])['Companies'].isin(companies_list)]['CAGR(10years)']), 
			'Initial Investment' : initial_investment, 
			'Additional Yearly Investment' : additional,
			'Value After 10 Years' : returns})

col1, col2, col3, col4 = st.columns(4)
col1.metric("Total Initial", str(human_format(updated['Initial Investment'].sum())))
col2.metric("Total Additional", str(human_format(updated['Additional Yearly Investment'].sum())))
col3.metric("Total After 10 Years", str(human_format(updated['Value After 10 Years'].sum())))
col4.metric("Grown Times", str(human_format(round(sum(updated['Value After 10 Years']) / (sum(updated['Initial Investment']) + sum(updated['Additional Yearly Investment']) * 10),0))))
